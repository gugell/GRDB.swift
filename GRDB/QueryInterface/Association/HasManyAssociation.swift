...
extension TypedRequest {
    public func including<Right>(_ association: HasManyAssociation<Fetched, Right>) -> GraphRequest<Self, Right> {
        return GraphRequest(leftRequest: self, association: association)
    }
}

extension TableMapping {
    public static func including<Right>(_ association: HasManyAssociation<Self, Right>) -> GraphRequest<QueryInterfaceRequest<Self>, Right> {
        return all().including(association)
    }
}

public struct GraphRequest<LeftRequest: TypedRequest, Right> {
    var leftRequest: LeftRequest
    let association: HasManyAssociation<LeftRequest.Fetched, Right>
}

extension GraphRequest where LeftRequest.Fetched: RowConvertible & TableMapping, Right: RowConvertible {
    public func fetchAll(_ db: Database) throws -> [(LeftRequest.Fetched, [Right])] {
        if association.rightColumns.count == 1 {
            let rightColumn = association.rightColumns[0]
            var result: [(LeftRequest.Fetched, [Right])] = []
            var leftPrimaryKeys: [DatabaseValue] = []
            var resultIndexes : [DatabaseValue: Int] = [:]
            
            let rowPrimaryKeyValue = try LeftRequest.Fetched.rowPrimaryKeyValue(db)
            ...
            while let (index, leftRow) = try leftCursor.next() {
                ...
                result.append((LeftRequest.Fetched(row: leftRow), []))
            }
