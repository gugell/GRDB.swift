...
class HasOneAssociationMappingsTests: GRDBTestCase {
    ...
    func testInferredForeignKey() throws {
        ...
        struct Parent : TableMapping, RowConvertible {
            ...
            static let children = hasOne(Child.self)
        }
        ...
        try dbQueue.inDatabase { db in
            let mapping = try Parent.children.mapping(db)
            assertEqual(mapping, [(left: "id", right: "parentId")])
        }
    }
    ...
    func testSingleRightColumn() throws {
        ...
        struct Parent : TableMapping, RowConvertible {
            ...
            // The tested association
            static let children = hasOne(Child.self, from: "parentId")
        }
